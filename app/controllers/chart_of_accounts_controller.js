...
const accounts__model       = db.accounts__model;
    ...
    try {
        ...
        const get_subsidiary_ledger = async(accounts_id) => {
            const coa_data = await chart_of_accounts_model.findAll({
                include: [
                    {
                        model: accounts_link_model,
                        attributes: ['accounts_link_code', 'accounts_link_name'],
                        association: chart_of_accounts_model.hasOne(accounts_link_model, {
                            foreignKey : 'accounts_link_id',
                            sourceKey : "chart_of_accounts_accounts_link_id",
                            required:false
                        })
                    }
                ],
            ...
            const data = await Promise.all(coa_data.map(async (row) => ({
                ...
                chart_of_accounts_accounts_link_id  : row.chart_of_accounts_accounts_link_id,
                chart_of_accounts_accounts_link     : row.accounts_link <= 0 ? '' : row.accounts_link.accounts_link_name,
...
// Get Chart of Accounts Subsidiary Ledger
exports.get_chart_of_accounts_subsidiary_ledger = async (req, res) => {
    try {
        const coa_data = await chart_of_accounts_model.findAll({
            include: [
                {
...
exports.get_chart_of_accounts_category = async (req, res) => {
    try {
        const data = await chart_of_accounts_model.findAll({
            ...
            where: {
                chart_of_accounts_company: req.query.company,
                chart_of_accounts_accounts_category: req.query.category,
                chart_of_accounts_delete_status:0,
                chart_of_accounts_status:1
            },
        });

        const get_balance_data = async(type, data) => {
            const balance_data = await accounts_details_model.findOne({
                attributes: [
                    [sequelize.literal('(SUM(accounts_details_debit) - SUM(accounts_details_credit))'),'balance']
                ],
                where: {
                    accounts_details_company: req.query.company,
                    ...(
                        type== 'subsidiary_ledger' ?{accounts_details_subsidiary_ledger : data}
                        :(type== 'general_ledger')?{accounts_details_general_ledger : data}
                        :(type== 'control_group')?{accounts_details_control_group : data}
                        :(type== 'accounts_category')?{accounts_details_accounts_category : data}
                        :{}
                    ),
            ...
            const balance = Math.abs(balance_data.dataValues.balance || 0).toFixed(2);
        ...
        if(data.length > 0) {
            const coa_data = await Promise.all(data.map(async (row) => ({
                ...
                chart_of_accounts_status            : row.chart_of_accounts_status,
                chart_of_accounts_posting_status    : row.chart_of_accounts_posting_status,
                chart_of_accounts_balance           : await get_balance_data(row.chart_of_accounts_coa_status, row.chart_of_accounts_id),
            })));
...
// Get Chart of Accounts Category
exports.get_chart_of_accounts_accounts_link = async (req, res) => {
    try {
        const data = await chart_of_accounts_model.findOne({
            ...
            where: {
                chart_of_accounts_company: req.query.company,
                chart_of_accounts_accounts_link: req.query.accounts_link,
                chart_of_accounts_delete_status:0,
                chart_of_accounts_status:1
            },
        });
        ...
        const coa_data = {
            ...
            chart_of_accounts_accounts_link_id  : data.chart_of_accounts_accounts_link_id,
            chart_of_accounts_accounts_link     : data.accounts_link <= 0 ? '' : data.accounts_link.accounts_link_name,
            ...
            chart_of_accounts_balance           : await get_balance_data(data.chart_of_accounts_coa_status, data.chart_of_accounts_id),
        };
...
exports.chart_of_accounts_create = async (req, res) => {
    try {
        ...
        if(check_coa_code) {
            return res.send({
                status: "0",
                message: "Accounts Code Duplicate!",
                data: "",
            });
        }
        ...
        const getAccountsLink = async(type, data) => {
            if(type == 'code') {
                const get_data = await accounts_link_model.findOne({ where:{ accounts_link_id : data } });
        ...
        if(at_data){
            const chart_of_accounts = await chart_of_accounts_model.create({
                ...
                chart_of_accounts_accounts_link_id  : req.body.chart_of_accounts_accounts_link_id,
                chart_of_accounts_accounts_link     : req.body.chart_of_accounts_accounts_link_id>0?await getAccountsLink('code', req.body.chart_of_accounts_accounts_link_id):null,
            ...
            if(chart_of_accounts) {
                ...
                return res.send({
                    status: "1",
                    message: "Chart of Accounts Create!",
                    data: {
                        chart_of_accounts_id                : data.chart_of_accounts_id ,
                        chart_of_accounts_code              : data.chart_of_accounts_code,
                        chart_of_accounts_name              : data.chart_of_accounts_name,
                        ...
                        chart_of_accounts_accounts_type     : data.chart_of_accounts_accounts_type,
                        chart_of_accounts_coa_status        : data.chart_of_accounts_coa_status,
                        chart_of_accounts_link              : data.chart_of_accounts_accounts_link_id,
                        chart_of_accounts_link              : data.chart_of_accounts_accounts_link,
                        chart_of_accounts_status            : data.chart_of_accounts_status,
                        chart_of_accounts_posting_status    : data.chart_of_accounts_posting_status,
                    }
                });
    ...
    try {
        ...
        if(at_data){
            ...
            if(chart_of_accounts) {
                ...
                return res.send({
                    status: "1",
                    message: "Chart of Accounts Update!",
                    data: {
                        chart_of_accounts_id                : data.chart_of_accounts_id ,
                        chart_of_accounts_code              : data.chart_of_accounts_code,
                        chart_of_accounts_name              : data.chart_of_accounts_name,
