    ...
    try {
        ...
        if (data.length > 0) {
            const branch_data = await Promise.all(data.map(async (row) => ({
                ...
                milestore: row.milestore,
                commencement_date: row.commencement_date,
                handing_over_date: row.handing_over_date,
                percentage_of_claiming_invoice: row.percentage_of_claiming_invoice,
                attachment: row.attachment,
                attach_quotation_and_invoice: row.attach_quotation_and_invoice,
...
exports.branch_create = async (req, res) => {
    try {
        ...
        const data = await branch_model.create({
            ...
            milestore: req.body.milestore,
            commencement_date: req.body.commencement_date,
            handing_over_date: req.body.handing_over_date,
            percentage_of_claiming_invoice: req.body.percentage_of_claiming_invoice,
            attachment: req.body.attachment,
            attach_quotation_and_invoice: req.body.attach_quotation_and_invoice,
