    ...
    try {
        ...
        if(data) {
            ...
            const company_info = await company_model.findOne({
                where: {
                    company_id: data.accounts_company
                }
            });
            const company_data = {
            ...
            const branch_info = await branch_model.findOne({
                where: {
                    branch_id: data.accounts_branch || 'all'
                }
            });
            const branch_data = {
...
exports.voucher_update = async (req, res) => {
    try {
        ...
        if(voucherData) {
            ...
            const voucher_number    = voucherData.accounts_voucher_number;
        ...
        const get_ledger_coa = async(coa_id, coa_accounts) => {
            ...
            const coa_data = await Promise.all(get_data.map(async (row) => {
                ...
                return({
                    ...
                    chart_of_accounts_accounts_link              : row.chart_of_account.chart_of_accounts_accounts_link || '',
...
exports.balance_sheet = async (req, res) => {
    try {
        ...
        const get_cat_balance = async(data) => {
            ...
            const c_month_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_cg'?(parseFloat(current_month_data.dataValues.balance || 0)+parseFloat(c_month_income_expenditure_balance)).toFixed(2):current_month_data.dataValues.balance;
            const l_month_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_cg'?(parseFloat(last_month_data.dataValues.balance || 0)+parseFloat(l_month_income_expenditure_balance)).toFixed(2):last_month_data.dataValues.balance;
            const c_year_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_cg'?(parseFloat(current_year_data.dataValues.balance || 0)+parseFloat(c_year_income_expenditure_balance)).toFixed(2):current_year_data.dataValues.balance;
            const l_year_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_cg'?(parseFloat(last_year_data.dataValues.balance || 0)+parseFloat(l_year_income_expenditure_balance)).toFixed(2):last_year_data.dataValues.balance;
        ...
        const get_gl_balance = async(data) => {
            ...
            const c_month_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_gl'?(parseFloat(current_month_data.dataValues.balance || 0)+parseFloat(c_month_income_expenditure_balance)).toFixed(2):current_month_data.dataValues.balance;
            const l_month_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_gl'?(parseFloat(last_month_data.dataValues.balance || 0)+parseFloat(l_month_income_expenditure_balance)).toFixed(2):last_month_data.dataValues.balance;
            const c_year_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_gl'?(parseFloat(current_year_data.dataValues.balance || 0)+parseFloat(c_year_income_expenditure_balance)).toFixed(2):current_year_data.dataValues.balance;
            const l_year_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_gl'?(parseFloat(last_year_data.dataValues.balance || 0)+parseFloat(l_year_income_expenditure_balance)).toFixed(2):last_year_data.dataValues.balance;
        ...
        const get_sl_balance = async(data) => {
            ...
            const c_month_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_sl'?(parseFloat(current_month_data.dataValues.balance || 0)+parseFloat(c_month_income_expenditure_balance)).toFixed(2):current_month_data.dataValues.balance;
            const l_month_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_sl'?(parseFloat(last_month_data.dataValues.balance || 0)+parseFloat(l_month_income_expenditure_balance)).toFixed(2):last_month_data.dataValues.balance;
            const c_year_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_sl'?(parseFloat(current_year_data.dataValues.balance || 0)+parseFloat(c_year_income_expenditure_balance)).toFixed(2):current_year_data.dataValues.balance;
            const l_year_balance = get_coa.chart_of_accounts_accounts_link == 'income_expenditure_sl'?(parseFloat(last_year_data.dataValues.balance || 0)+parseFloat(l_year_income_expenditure_balance)).toFixed(2):last_year_data.dataValues.balance;
...
exports.trial_balance = async (req, res) => {
    try {
        ...
        const get_general_ledger_balance = async(type, data) => {
            ...
            const balance_data = parseFloat(get_balance.dataValues.balance) || 0;
        ...
        const total_balance_data = {
            debit_balance   : Math.abs(trial_balance_data.reduce((acc, item) => acc + parseFloat(item.balance.debit_balance), 0)).toFixed(2),
            credit_balance  : Math.abs(trial_balance_data.reduce((acc, item) => acc + parseFloat(item.balance.credit_balance), 0)).toFixed(2),
        }

        return res.send({
            status: "1",
            message: "Trial Balance Find Successfully!",
            data: {
...
exports.receipts_payments = async (req, res) => {
    try {
        ...
        const cash_bank_opening_coa_data = await Promise.all(cash_bank_coa.map(async (row) => ({
            ...
            chart_of_accounts_accounts_link              : row.chart_of_accounts_accounts_link || '',
    ...
    try {
        ...
        const debit_coa_data = await accounts_details_model.findAll({
            ...
            include: [
                {
                    model: chart_of_accounts_model,
                    attributes: ['chart_of_accounts_id', 'chart_of_accounts_accounts_link'],
                    association: accounts_details_model.hasOne(chart_of_accounts_model, {
                        foreignKey : 'chart_of_accounts_id',
                        sourceKey : "accounts_details_general_ledger",
                        required:false
...
exports.add_cash_bank_balance = async (req, res) => {
    try {
        const token = req.headers["x-access-token"];
        jwt.verify(token, config.secret, (err, decoded) => {
            req.user_id = decoded.user_id;
        ...
        const getAccountsCatData = async(type, data) => {
            if(type == 'at') {
                ...
                const get_data = await chart_of_accounts_model.findOne({ where:{ chart_of_accounts_accounts_link : data } });
        ...
        const rv_voucher_type      = 1;
        const rv_voucher_type_code = await getVoucherType('code', rv_voucher_type);
        const rv_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:rv_voucher_type}})+1;
        const rv_voucher_number = rv_voucher_type_code+"-"+rv_voucher_count.toString().padStart(7, '0');

        const rv_narration = 'Fund Received';

        const rv_data = {
            accounts_company        : req.body.company,
            accounts_branch         : req.body.branch,
            accounts_posting_date   : posting_date,
            accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
            accounts_posting_year   : new Date(posting_date).getFullYear(),
            accounts_voucher_type   : rv_voucher_type,
            accounts_voucher_number : rv_voucher_number,
            accounts_narration      : rv_narration,
            accounts_total_debit    : req.body.amount,
            accounts_total_credit   : req.body.amount,
            accounts_status         : 1,
            accounts_create_by      : user_id,
        }
        const rv_accounts_data = await accounts_model.create(rv_data);
        const rv_data_details = [
            {
                accounts_details_company                : req.body.company,
                accounts_details_branch                 : req.body.branch,
                accounts_details_accounts               : rv_accounts_data.accounts_id,
                accounts_details_posting_date           : posting_date,
                accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                accounts_details_voucher_type           : rv_voucher_type,
                accounts_details_voucher_number         : rv_voucher_number,
                accounts_details_narration              : rv_narration,
                accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl', req.body.subsidiary_ledger)))),
                accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl', req.body.subsidiary_ledger))),
                accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl', req.body.subsidiary_ledger)),
                accounts_details_general_ledger         : await getAccountsCatData('gl', req.body.subsidiary_ledger),
                accounts_details_subsidiary_ledger      : req.body.subsidiary_ledger,
                accounts_details_debit                  : req.body.amount,
                accounts_details_credit                 : '0.00',
                accounts_details_status                 : 1,
                accounts_details_create_by              : user_id
            },
            {
                ...
                accounts_details_voucher_number         : rv_voucher_number,
                accounts_details_narration              : rv_narration,
                accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'investments_sl'))))),
                accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'investments_sl')))),
                accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'investments_sl'))),
                accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'investments_sl')),
                accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'investments_sl'),
                accounts_details_debit                  : '0.00',
                accounts_details_credit                 : req.body.amount,
                accounts_details_status                 : 1,
                accounts_details_create_by              : user_id
            }
            
        ];
        const rv_accounts_details_data = await accounts_details_model.bulkCreate(rv_data_details);
...
// add cash bank balance
exports.cash_bank_balance_withdraw = async (req, res) => {
    try {
        const token = req.headers["x-access-token"];
        jwt.verify(token, config.secret, (err, decoded) => {
            req.user_id = decoded.user_id;
        ...
        const pv_voucher_type      = 2;
        const pv_voucher_type_code = await getVoucherType('code', pv_voucher_type);
        const pv_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:pv_voucher_type}})+1;
        const pv_voucher_number = pv_voucher_type_code+"-"+pv_voucher_count.toString().padStart(7, '0');

        const pv_narration = 'Balance Withdraw';
        ...
        const pv_data = {
            ...
            accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
            accounts_posting_year   : new Date(posting_date).getFullYear(),
            accounts_voucher_type   : pv_voucher_type,
            accounts_voucher_number : pv_voucher_number,
            accounts_narration      : pv_narration,
            accounts_total_debit    : req.body.amount,
            accounts_total_credit   : req.body.amount,
            accounts_status         : 1,
            accounts_create_by      : user_id,
        }
        const pv_accounts_data = await accounts_model.create(pv_data);
        const pv_data_details = [
            
            {
                accounts_details_company                : req.body.company,
                accounts_details_branch                 : req.body.branch,
                accounts_details_accounts               : pv_accounts_data.accounts_id,
                accounts_details_posting_date           : posting_date,
                accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                accounts_details_voucher_type           : pv_voucher_type,
                accounts_details_voucher_number         : pv_voucher_number,
                accounts_details_narration              : pv_narration,
                accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'investments_sl'))))),
                accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'investments_sl')))),
                accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'investments_sl'))),
                accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'investments_sl')),
        ...
        const pv_accounts_details_data = await accounts_details_model.bulkCreate(pv_data_details);
...
// Day Close Authorize
exports.day_close_authorize = async (req, res) => {
    try {
        const posting_date          = req.body.to_date;

        const posting_check = await accounts_model.findOne({
            where: {
                accounts_posting_status :1,
                accounts_posting_date   : posting_date
            },
            order: [
                ['accounts_posting_date', 'ASC']
        ...
        if(posting_check) {
            return res.send({
                status  : "0",
                message : "Already Authorized, Remove Previous Voucher!",
                data    : posting_check
            });
        }
        ...
        const purchase_data = await purchase_model.findOne({
            ...
            where:{
                purchase_company        : req.body.company,
                ...(req.body.branch == 'all' ?{}:{ purchase_branch : req.body.branch}),
                purchase_date           : {[Op.between]: [req.body.from_date, req.body.to_date]},
                purchase_status         : 1,
        ...
        const purchase_payment_data = await supplier_payment_model.findOne({
            ...
            where:{
                supplier_payment_company        : req.body.company,
                ...(req.body.branch == 'all' ?{}:{ supplier_payment_branch : req.body.branch}),
                supplier_payment_date           : {[Op.between]: [req.body.from_date, req.body.to_date]},
        ...
        const purchase_return_data = await purchase_return_model.findOne({
            ...
            where:{
                purchase_return_company        : req.body.company,
                ...(req.body.branch == 'all' ?{}:{ purchase_return_branch : req.body.branch}),
                purchase_return_date           : {[Op.between]: [req.body.from_date, req.body.to_date]},
        ...
        const purchase_return_payment_data = await supplier_payment_return_model.findOne({
            ...
            where:{
                supplier_payment_return_company        : req.body.company,
                ...(req.body.branch == 'all' ?{}:{ supplier_payment_return_branch : req.body.branch}),
                supplier_payment_return_date           : {[Op.between]: [req.body.from_date, req.body.to_date]},
        ...
        const sales_data = await sales_model.findOne({
            ...
            where:{
                sales_company        : req.body.company,
                ...(req.body.branch == 'all' ?{}:{ sales_branch : req.body.branch}),
                sales_date           : {[Op.between]: [req.body.from_date, req.body.to_date]},
                sales_status         : 1,
        ...
        const sales_payment_data = await customer_payment_model.findOne({
            ...
            where:{
                customer_payment_company        : req.body.company,
                ...(req.body.branch == 'all' ?{}:{ customer_payment_branch : req.body.branch}),
                customer_payment_date           : {[Op.between]: [req.body.from_date, req.body.to_date]},
        ...
        const sales_return_data = await sales_return_model.findOne({
            ...
            where:{
                sales_return_company        : req.body.company,
                ...(req.body.branch == 'all' ?{}:{ sales_return_branch : req.body.branch}),
                sales_return_date           : {[Op.between]: [req.body.from_date, req.body.to_date]},
        ...
        const sales_return_payment_data = await customer_payment_return_model.findOne({
            ...
            where:{
                customer_payment_return_company        : req.body.company,
                ...(req.body.branch == 'all' ?{}:{ customer_payment_return_branch : req.body.branch}),
                customer_payment_return_date           : {[Op.between]: [req.body.from_date, req.body.to_date]},
        ...
        const getReceiptAmount = async(type)=> {
            ...
            if(type == 'cash') {
                const cash_bank_data = await chart_of_accounts_model.findOne({
                    where:{
                        chart_of_accounts_company: req.body.company,
                        chart_of_accounts_accounts_link: 'cash_in_hand',
                        chart_of_accounts_status: 1,
                        chart_of_accounts_delete_status: 0
                    }
        ...
        const rv_fn_cash_voucher_type      = 1;
        const rv_fn_cash_voucher_type_code = await getVoucherType('code', rv_fn_cash_voucher_type);
        const rv_fn_cash_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:rv_fn_cash_voucher_type}})+1;
        const rv_fn_cash_voucher_number = rv_fn_cash_voucher_type_code+"-"+rv_fn_cash_voucher_count.toString().padStart(7, '0');
        
        const rv_fn_bank_voucher_type      = 1;
        const rv_fn_bank_voucher_type_code = await getVoucherType('code', rv_fn_bank_voucher_type);
        const rv_fn_bank_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:rv_fn_bank_voucher_type}})+posting_data.fund_need.cash > 0?2:1;
        const rv_fn_bank_voucher_number = rv_fn_bank_voucher_type_code+"-"+rv_fn_bank_voucher_count.toString().padStart(7, '0');
        
        const rv_cash_voucher_type      = 1;
        const rv_cash_voucher_type_code = await getVoucherType('code', rv_cash_voucher_type);
        const rv_cash_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:rv_cash_voucher_type}})+posting_data.fund_need.cash && posting_data.fund_need.bank > 0?3:(posting_data.fund_need.cash>0)?2:(posting_data.fund_need.bank>0)?2:1;
        const rv_cash_voucher_number = rv_cash_voucher_type_code+"-"+rv_cash_voucher_count.toString().padStart(7, '0');

        const rv_bank_voucher_type      = 1;
        const rv_bank_voucher_type_code = await getVoucherType('code', rv_bank_voucher_type);
        const rv_bank_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:rv_bank_voucher_type}})+4;
        const rv_bank_voucher_number = rv_bank_voucher_type_code+"-"+rv_bank_voucher_count.toString().padStart(7, '0');

        const pv_cash_voucher_type      = 2;
        const pv_cash_voucher_type_code = await getVoucherType('code', pv_cash_voucher_type);
        const pv_cash_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:pv_cash_voucher_type}})+1;
        const pv_cash_voucher_number = pv_cash_voucher_type_code+"-"+pv_cash_voucher_count.toString().padStart(7, '0');
        
        const pv_bank_voucher_type      = 2;
        const pv_bank_voucher_type_code = await getVoucherType('code', pv_bank_voucher_type);
        const pv_bank_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:pv_bank_voucher_type}})+2;
        const pv_bank_voucher_number = pv_bank_voucher_type_code+"-"+pv_bank_voucher_count.toString().padStart(7, '0');
        
        const jv_pd_voucher_type      = 3;
        const jv_pd_voucher_type_code = await getVoucherType('code', jv_pd_voucher_type);
        const jv_pd_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:jv_pd_voucher_type}})+1;
        const jv_pd_voucher_number = jv_pd_voucher_type_code+"-"+jv_pd_voucher_count.toString().padStart(7, '0');

        const jv_sd_voucher_type      = 3;
        const jv_sd_voucher_type_code = await getVoucherType('code', jv_sd_voucher_type);
        const jv_sd_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:jv_sd_voucher_type}})+2;
        const jv_sd_voucher_number = jv_sd_voucher_type_code+"-"+jv_sd_voucher_count.toString().padStart(7, '0');
        
        const jv_prd_voucher_type      = 3;
        const jv_prd_voucher_type_code = await getVoucherType('code', jv_prd_voucher_type);
        const jv_prd_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:jv_prd_voucher_type}})+3;
        const jv_prd_voucher_number = jv_prd_voucher_type_code+"-"+jv_prd_voucher_count.toString().padStart(7, '0');

        const jv_srd_voucher_type      = 3;
        const jv_srd_voucher_type_code = await getVoucherType('code', jv_srd_voucher_type);
        const jv_srd_voucher_count = await accounts_model.count({where:{accounts_company:req.body.company,accounts_branch:req.body.branch, accounts_voucher_type:jv_srd_voucher_type}})+4;
        const jv_srd_voucher_number = jv_srd_voucher_type_code+"-"+jv_srd_voucher_count.toString().padStart(7, '0');

        const rv_fn_cash_narration = 'Cash Fund Received';
        const rv_fn_bank_narration = 'Bank Fund Received';
        const rv_cash_narration = 'Cash Collection';
        const rv_bank_narration = 'Bank Collection';
        const pv_cash_narration = 'Cash Payment';
        const pv_bank_narration = 'Bank Payment';

        const jv_pd_narration   = 'Purchase Due';
        const jv_sd_narration  = 'Sales Due';
        const jv_prd_narration   = 'Purchase Return Due';
        const jv_srd_narration   = 'Sales Return Due';

        const getCashBankReceiptAmount = async(type, data)=> {
            if(type == 'sales_payment_data') {
                const sales_payment_data = await customer_payment_model.findOne({
                    attributes: [
                        [sequelize.literal('(SUM(customer_payment_paid))'),'paid_amount']
                    ],
                ...
                return parseFloat(sales_payment_data.dataValues.paid_amount || 0).toFixed(2);
            } else if(type == 'sales_due_payment_data') {
                const sales_due_payment_data = await customer_payment_model.findOne({
                    attributes: [
                        [sequelize.literal('(SUM(customer_payment_paid))'),'due_payment']
                    ],
                ...
                return parseFloat(sales_due_payment_data.dataValues.due_payment || 0).toFixed(2);
            } else if(type == 'purchase_return_payment_data') {
                const purchase_return_payment_data = await supplier_payment_return_model.findOne({
                    attributes: [
                        [sequelize.literal('(SUM(supplier_payment_return_paid))'),'return_paid_amount']
                    ],
                ...
                return parseFloat(purchase_return_payment_data.dataValues.return_paid_amount || 0).toFixed(2);
            } else if(type == 'purchase_return_due_payment_data') {
                const purchase_return_due_payment_data = await supplier_payment_return_model.findOne({
                    attributes: [
                        [sequelize.literal('(SUM(supplier_payment_return_paid))'),'return_due_payment']
                    ],
                ...
                return parseFloat(purchase_return_due_payment_data.dataValues.return_due_payment || 0).toFixed(2);
            }
        }

        const getCashBankPaymentAmount = async(type, data)=> {
            if(type == 'purchase_payment_data') {
                const purchase_payment_data = await supplier_payment_model.findOne({
                    attributes: [
                        [sequelize.literal('(SUM(supplier_payment_paid))'),'paid_amount']
                    ],
                ...
                return parseFloat(purchase_payment_data.dataValues.paid_amount || 0).toFixed(2);
            } else if(type == 'purchase_due_payment_data') {
                const purchase_due_payment_data = await supplier_payment_model.findOne({
                    attributes: [
                        [sequelize.literal('(SUM(supplier_payment_paid))'),'due_payment']
                    ],
                ...
                return parseFloat(purchase_due_payment_data.dataValues.due_payment || 0).toFixed(2);
            }  else if(type == 'sales_return_payment_data') {
                const sales_return_payment_data = await customer_payment_return_model.findOne({
                    attributes: [
                        [sequelize.literal('(SUM(customer_payment_return_paid))'),'return_paid_amount']
                    ],
                ...
                return parseFloat(sales_return_payment_data.dataValues.return_paid_amount || 0).toFixed(2);
            }  else if(type == 'sales_return_due_payment_data') {
                const sales_return_due_payment_data = await customer_payment_return_model.findOne({
                    attributes: [
                        [sequelize.literal('(SUM(customer_payment_return_paid))'),'return_due_payment']
                    ],
                ...
                return parseFloat(sales_return_due_payment_data.dataValues.return_due_payment || 0).toFixed(2);
            }
        }

        if(posting_data.fund_need.cash > 0) {
            const rv_fn_cash_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : rv_fn_cash_voucher_type,
                accounts_voucher_number : rv_fn_cash_voucher_number,
                accounts_narration      : rv_fn_cash_narration,
                accounts_total_debit    : parseFloat(posting_data.fund_need.cash),
                accounts_total_credit   : parseFloat(posting_data.fund_need.cash),
                accounts_posting_status : 1,
                accounts_status         : 1,
                accounts_create_by      : user_id,
            }
            const rv_fn_cash_accounts_data = await accounts_model.create(rv_fn_cash_data);
            
            const rv_fn_cash_data_details = [
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_fn_cash_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : rv_fn_cash_voucher_type,
                    accounts_details_voucher_number         : rv_fn_cash_voucher_number,
                    accounts_details_narration              : rv_fn_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'cash'),
                    accounts_details_debit                  : posting_data.fund_need.cash,
                    accounts_details_credit                 : '0.00',
                    accounts_details_posting_status         : 1,
                    accounts_details_status                 : 1,
                    accounts_details_create_by              : user_id,
                },
                {
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'investments_sl'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : posting_data.fund_need.cash,
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }
            ];
            const rv_fn_cash_accounts_details_data = await accounts_details_model.bulkCreate(rv_fn_cash_data_details);
        }
        
        if(posting_data.fund_need.bank > 0) {
            const rv_fn_bank_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : rv_fn_bank_voucher_type,
                accounts_voucher_number : rv_fn_bank_voucher_number,
                accounts_narration      : rv_fn_bank_narration,
                accounts_total_debit    : parseFloat(posting_data.fund_need.bank),
                accounts_total_credit   : parseFloat(posting_data.fund_need.bank),
                accounts_posting_status : 1,
                accounts_status         : 1,
                accounts_create_by      : user_id,
            }
            const rv_fn_bank_accounts_data = await accounts_model.create(rv_fn_bank_data);
            
            const rv_fn_bank_data_details = [
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_fn_bank_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : rv_fn_bank_voucher_type,
                    accounts_details_voucher_number         : rv_fn_bank_voucher_number,
                    accounts_details_narration              : rv_fn_bank_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'bank'),
                    accounts_details_debit                  : posting_data.fund_need.bank,
                    accounts_details_credit                 : '0.00',
                    accounts_details_posting_status         : 1,
                    accounts_details_status                 : 1,
                    accounts_details_create_by              : user_id,
                },
                {
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'investments_sl'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : posting_data.fund_need.bank,
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }
            ];
            const rv_fn_bank_accounts_details_data = await accounts_details_model.bulkCreate(rv_fn_bank_data_details);
        }

        if(posting_data.receipt_payment.receipt.cash>0) {
            const rv_cash_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : rv_cash_voucher_type,
                accounts_voucher_number : rv_cash_voucher_number,
                accounts_narration      : rv_cash_narration,
                accounts_total_debit    : parseFloat(posting_data.receipt_payment.receipt.cash),
                accounts_total_credit   : parseFloat(posting_data.receipt_payment.receipt.cash),
                accounts_posting_status : 1,
                accounts_status         : 1,
                accounts_create_by      : user_id,
            }
            const rv_cash_accounts_data = await accounts_model.create(rv_cash_data);
            const rv_cash_data_details = [
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_cash_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : rv_cash_voucher_type,
                    accounts_details_voucher_number         : rv_cash_voucher_number,
                    accounts_details_narration              : rv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'cash'),
                    accounts_details_debit                  : posting_data.receipt_payment.receipt.cash,
                    accounts_details_credit                 : '0.00',
                    accounts_details_posting_status         : 1,
                    accounts_details_status                 : 1,
                    accounts_details_create_by              : user_id,
                },
                await getCashBankReceiptAmount('sales_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_cash_accounts_data.accounts_id,
                    ...
                    accounts_details_voucher_number         : rv_cash_voucher_number,
                    accounts_details_narration              : rv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_collection'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_collection')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_collection'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_collection')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_collection'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : await getCashBankReceiptAmount('sales_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankReceiptAmount('sales_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_cash_accounts_data.accounts_id,
                    ...
                    accounts_details_voucher_number         : rv_cash_voucher_number,
                    accounts_details_narration              : rv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_due'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_due')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_due'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_due')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_due'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : await getCashBankReceiptAmount('sales_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankReceiptAmount('purchase_return_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_cash_accounts_data.accounts_id,
                    ...
                    accounts_details_voucher_number         : rv_cash_voucher_number,
                    accounts_details_narration              : rv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_return'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : await getCashBankReceiptAmount('purchase_return_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankReceiptAmount('purchase_return_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_cash_accounts_data.accounts_id,
                    ...
                    accounts_details_voucher_number         : rv_cash_voucher_number,
                    accounts_details_narration              : rv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return_due'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return_due')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return_due'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return_due')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_return_due'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : await getCashBankReceiptAmount('purchase_return_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{}
            ];
            const rv_cash_accounts_details_data = await accounts_details_model.bulkCreate(rv_cash_data_details);
        }

        if(posting_data.receipt_payment.receipt.bank>0) {
            const rv_bank_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : rv_bank_voucher_type,
                accounts_voucher_number : rv_bank_voucher_number,
                accounts_narration      : rv_bank_narration,
                accounts_total_debit    : parseFloat(posting_data.receipt_payment.receipt.bank),
                accounts_total_credit   : parseFloat(posting_data.receipt_payment.receipt.bank),
                accounts_status         : 1,
                accounts_posting_status : 1,
                accounts_create_by      : user_id,
            }
            const rv_bank_accounts_data = await accounts_model.create(rv_bank_data);
            const rv_bank_data_details = [
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_bank_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : rv_bank_voucher_type,
                    accounts_details_voucher_number         : rv_bank_voucher_number,
                    accounts_details_narration              : rv_bank_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'bank'),
                    accounts_details_debit                  : posting_data.receipt_payment.receipt.bank,
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                },
                await getCashBankReceiptAmount('sales_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_bank_accounts_data.accounts_id,
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_collection'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : await getCashBankReceiptAmount('sales_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankReceiptAmount('sales_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_bank_accounts_data.accounts_id,
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_due'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : await getCashBankReceiptAmount('sales_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankReceiptAmount('purchase_return_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_bank_accounts_data.accounts_id,
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_return'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : await getCashBankReceiptAmount('purchase_return_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankReceiptAmount('purchase_return_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : rv_bank_accounts_data.accounts_id,
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_return_due'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : await getCashBankReceiptAmount('purchase_return_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{}
            ];
            const rv_bank_accounts_details_data = await accounts_details_model.bulkCreate(rv_bank_data_details);
        }

        if(posting_data.receipt_payment.payment.cash>0) {
            const pv_cash_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : pv_cash_voucher_type,
                accounts_voucher_number : pv_cash_voucher_number,
                accounts_narration      : pv_cash_narration,
                accounts_total_debit    : parseFloat(posting_data.receipt_payment.payment.cash),
                accounts_total_credit   : parseFloat(posting_data.receipt_payment.payment.cash),
                accounts_status         : 1,
                accounts_posting_status : 1,
                accounts_create_by      : user_id,
            }
            const pv_cash_accounts_data = await accounts_model.create(pv_cash_data);
            const pv_cash_data_details = [
                await getCashBankPaymentAmount('purchase_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : pv_cash_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : pv_cash_voucher_type,
                    accounts_details_voucher_number         : pv_cash_voucher_number,
                    accounts_details_narration              : pv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_payment'),
                    accounts_details_debit                  : await getCashBankPaymentAmount('purchase_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankPaymentAmount('purchase_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : pv_cash_accounts_data.accounts_id,
                    ...
                    accounts_details_voucher_number         : pv_cash_voucher_number,
                    accounts_details_narration              : pv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_due'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_due')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_due'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_due')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_due'),
                    accounts_details_debit                  : await getCashBankPaymentAmount('purchase_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankPaymentAmount('sales_return_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : pv_cash_accounts_data.accounts_id,
                    ...
                    accounts_details_voucher_number         : pv_cash_voucher_number,
                    accounts_details_narration              : pv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_return'),
                    accounts_details_debit                  : await getCashBankPaymentAmount('sales_return_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankPaymentAmount('sales_return_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : pv_cash_accounts_data.accounts_id,
                    ...
                    accounts_details_voucher_number         : pv_cash_voucher_number,
                    accounts_details_narration              : pv_cash_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return_due'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return_due')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return_due'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return_due')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_return_due'),
                    accounts_details_debit                  : await getCashBankPaymentAmount('sales_return_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'cash'))),
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                {
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'cash'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : posting_data.receipt_payment.payment.cash,
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }
            ];
            const pv_cash_accounts_details_data = await accounts_details_model.bulkCreate(pv_cash_data_details);
        }
        
        if(posting_data.receipt_payment.payment.bank>0) {
            const pv_bank_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : pv_bank_voucher_type,
                accounts_voucher_number : pv_bank_voucher_number,
                accounts_narration      : pv_bank_narration,
                accounts_total_debit    : parseFloat(posting_data.receipt_payment.payment.bank),
                accounts_total_credit   : parseFloat(posting_data.receipt_payment.payment.bank),
                accounts_status         : 1,
                accounts_posting_status : 1,
                accounts_create_by      : user_id,
            }
            const pv_bank_accounts_data = await accounts_model.create(pv_bank_data);
            const pv_bank_data_details = [
                await getCashBankPaymentAmount('purchase_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : pv_bank_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : pv_bank_voucher_type,
                    accounts_details_voucher_number         : pv_bank_voucher_number,
                    accounts_details_narration              : pv_bank_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_payment'),
                    accounts_details_debit                  : await getCashBankPaymentAmount('purchase_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankPaymentAmount('purchase_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : pv_bank_accounts_data.accounts_id,
                    ...
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_due')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_due'),
                    accounts_details_debit                  : await getCashBankPaymentAmount('purchase_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankPaymentAmount('sales_return_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : pv_bank_accounts_data.accounts_id,
                    ...
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_return'),
                    accounts_details_debit                  : await getCashBankPaymentAmount('sales_return_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                await getCashBankPaymentAmount('sales_return_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank')))>0?
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : pv_bank_accounts_data.accounts_id,
                    ...
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return_due')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_return_due'),
                    accounts_details_debit                  : await getCashBankPaymentAmount('sales_return_due_payment_data', await getAccountsCatData('gl',await getAccountsCatData('sl', 'bank'))),
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }:{},
                {
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'bank'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : posting_data.receipt_payment.payment.bank,
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }
            ];
            const pv_bank_accounts_details_data = await accounts_details_model.bulkCreate(pv_bank_data_details);
        }

        if(posting_data.purchase.purchase_due_amount >0) {
            const jv_pd_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : jv_pd_voucher_type,
                accounts_voucher_number : jv_pd_voucher_number,
                accounts_narration      : jv_pd_narration,
                accounts_total_debit    : parseFloat(posting_data.purchase.purchase_due_amount),
                accounts_total_credit   : parseFloat(posting_data.purchase.purchase_due_amount),
                accounts_status         : 1,
                accounts_posting_status : 1,
                accounts_create_by      : user_id,
            }
            const jv_pd_accounts_data = await accounts_model.create(jv_pd_data);
            const jv_pd_data_details = [
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : jv_pd_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : jv_pd_voucher_type,
                    accounts_details_voucher_number         : jv_pd_voucher_number,
                    accounts_details_narration              : jv_pd_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_payment')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_payment'),
                    accounts_details_debit                  : posting_data.purchase.purchase_due_amount,
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id
                },
                {
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_due'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : posting_data.purchase.purchase_due_amount,
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id
                }
            ];
            const jv_pd_accounts_details_data = await accounts_details_model.bulkCreate(jv_pd_data_details);
        }
        
        if(posting_data.sales.sales_due_amount >0) {
            const jv_sd_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : jv_sd_voucher_type,
                accounts_voucher_number : jv_sd_voucher_number,
                accounts_narration      : jv_sd_narration,
                accounts_total_debit    : parseFloat(posting_data.sales.sales_due_amount),
                accounts_total_credit   : parseFloat(posting_data.sales.sales_due_amount),
                accounts_status         : 1,
                accounts_posting_status : 1,
                accounts_create_by      : user_id,
            }
            const jv_sd_accounts_data = await accounts_model.create(jv_sd_data);
            const jv_sd_data_details = [
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : jv_sd_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : jv_sd_voucher_type,
                    accounts_details_voucher_number         : jv_sd_voucher_number,
                    accounts_details_narration              : jv_sd_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_due'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_due')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_due'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_due')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_due'),
                    accounts_details_debit                  : posting_data.sales.sales_due_amount,
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id
                },
                {
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_collection'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : posting_data.sales.sales_due_amount,
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id
            ...
            const jv_sd_accounts_details_data = await accounts_details_model.bulkCreate(jv_sd_data_details);
        }

        if(posting_data.purchase.purchase_return_due_amount >0) {
            const jv_prd_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : jv_prd_voucher_type,
                accounts_voucher_number : jv_prd_voucher_number,
                accounts_narration      : jv_prd_narration,
                accounts_total_debit    : parseFloat(posting_data.purchase.purchase_return_due_amount),
                accounts_total_credit   : parseFloat(posting_data.purchase.purchase_return_due_amount),
                accounts_status         : 1,
                accounts_posting_status : 1,
                accounts_create_by      : user_id,
            }
            const jv_prd_accounts_data = await accounts_model.create(jv_prd_data);
            const jv_prd_data_details = [
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : jv_prd_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : jv_prd_voucher_type,
                    accounts_details_voucher_number         : jv_prd_voucher_number,
                    accounts_details_narration              : jv_prd_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return_due'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return_due')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return_due'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'purchase_return_due')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_return_due'),
                    accounts_details_debit                  : posting_data.purchase.purchase_return_due_amount,
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id
                },
                {
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'purchase_return'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : posting_data.purchase.purchase_return_due_amount,
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id,
                }
            ];
            const jv_prd_accounts_details_data = await accounts_details_model.bulkCreate(jv_prd_data_details);
        }

        if(posting_data.sales.sales_return_due_amount >0) {
            const jv_srd_data = {
                accounts_company        : req.body.company,
                accounts_branch         : req.body.branch,
                accounts_posting_date   : posting_date,
                accounts_posting_month  : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                accounts_posting_year   : new Date(posting_date).getFullYear(),
                accounts_voucher_type   : jv_srd_voucher_type,
                accounts_voucher_number : jv_srd_voucher_number,
                accounts_narration      : jv_srd_narration,
                accounts_total_debit    : parseFloat(posting_data.sales.sales_return_due_amount),
                accounts_total_credit   : parseFloat(posting_data.sales.sales_return_due_amount),
                accounts_status         : 1,
                accounts_posting_status : 1,
                accounts_create_by      : user_id,
            }
            const jv_srd_accounts_data = await accounts_model.create(jv_srd_data);
            const jv_srd_data_details = [
                {
                    accounts_details_company                : req.body.company,
                    accounts_details_branch                 : req.body.branch,
                    accounts_details_accounts               : jv_srd_accounts_data.accounts_id,
                    accounts_details_posting_date           : posting_date,
                    accounts_details_posting_month          : new Date(posting_date).toLocaleString('default',{month:'long'})+"-"+new Date(posting_date).getFullYear(),
                    accounts_details_posting_year           : new Date(posting_date).getFullYear(),
                    accounts_details_voucher_type           : jv_srd_voucher_type,
                    accounts_details_voucher_number         : jv_srd_voucher_number,
                    accounts_details_narration              : jv_srd_narration,
                    accounts_details_accounts_type          : await getAccountsCatData('at', await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return'))))),
                    accounts_details_accounts_category      : await getAccountsCatData('ac', await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return')))),
                    accounts_details_control_group          : await getAccountsCatData('cg', await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return'))),
                    accounts_details_general_ledger         : await getAccountsCatData('gl',await getAccountsCatData('sl', 'sales_return')),
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_return'),
                    accounts_details_debit                  : posting_data.sales.sales_return_due_amount,
                    accounts_details_credit                 : '0.00',
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id
                },
                {
                    ...
                    accounts_details_subsidiary_ledger      : await getAccountsCatData('sl', 'sales_return_due'),
                    accounts_details_debit                  : '0.00',
                    accounts_details_credit                 : posting_data.sales.sales_return_due_amount,
                    accounts_details_status                 : 1,
                    accounts_details_posting_status         : 1,
                    accounts_details_create_by              : user_id
                }
            ];
            const jv_srd_accounts_details_data = await accounts_details_model.bulkCreate(jv_srd_data_details);
        }

        const delete_zero_data = await accounts_details_model.destroy({
                where: {
                    accounts_details_voucher_type: 0
                }
            }
        );
        ...
        return res.send({
            status  : "1",
            message : "Day Close Authorize Successfully!",
            data    : ""
        });
